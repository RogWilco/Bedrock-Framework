...
//			'Windows'
		...
		switch($script) {
			case 'controller':
				$result .=
					...
					' */' . $n .
					'abstract class ' . $namespace . '_Control extends Bedrock_Control {' . $n .
					$t . '' . $n .
					'}' . $n .
					'?>';
				break;
			...
			case 'controller:index':
				$result .=
					...
					' * @updated ' . date('m/d/Y') . $n .
					' */' . $n .
					'class ' . $namespace . '_Control_Index extends ' . $namespace . '_Control {' . $n .
					$t . '/**' . $n .
			...
			case 'view':
				$result .=
					...
					' * @updated ' . date('m/d/Y') . $n .
					' */' . $n .
					'abstract class ' . $namespace . '_View extends Bedrock_View {' . $n .
					$t . '' . $n .
					'}' . $n .
					'?>';
				break;
			...
			case 'view:index':
				$result .=
					...
					' * @updated ' . date('m/d/Y') . $n .
					' */' . $n .
					'class ' . $namespace . '_View_Index extends ' . $namespace . '_View {' . $n .
					$t . '/**' . $n .
					...
					$t . $t . $t . 'Bedrock_Common_Logger::exception($ex);' . $n .
					$t . $t . $t . 'Bedrock_Common_Logger::logExit();' . $n .
					$t . $t . $t . 'throw new Bedrock_View_Exception(\'The view could not be rendered.\');' . $n .
					$t . $t . '}' . $n .
					$t . '}' . $n .
					'}' . $n .
					'?>';
	...
	/**
	 * Compares the current system requirement setting with the specified value.
	 *
				...
				if($_POST['database_type'] == 'mysql_5') {
					...
					if(!self::hasErrors(3)) {
						...
						try {
							...
							$connection = new PDO($dsn, self::getValue('database_username'), self::getValue('database_password'));
						}
