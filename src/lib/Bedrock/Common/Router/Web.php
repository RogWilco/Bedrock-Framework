...
/**
 * The main router object that handles web requests and loads the proper controller.
 * 
 * @package Bedrock
 ...
 * @updated 09/07/2020
 */
class Web extends \Bedrock\Common\Router {
	 ...
	 *
	 * @param array|string $delegationQueue a list of root controller classes in order of precedence for delegation
	 */
			...
			if($parts[0] == 'query' || substr($parts[0], 0, 6) == 'query:') {
				...
				if(!$specialCase) {
					...
					while($parts) {
						...
						$controller = self::_getController($this->_delegationQueue, $parts);
						if($controller) break;
	 ...
	 *
	 * @throws Exception if a controller cannot be instantiated
	 * @return mixed either the controller if it exists, or false otherwise
	 */
	protected static function _getController($delegationQueue, $route) {
		try {
			// Setup
			$result = false;
	 ...
	 *
	 * @throws Exception if delegation fails
	 * @return void
	 */
	private function _delegateToError() {
		try {
			// Setup
			$controller = false;
